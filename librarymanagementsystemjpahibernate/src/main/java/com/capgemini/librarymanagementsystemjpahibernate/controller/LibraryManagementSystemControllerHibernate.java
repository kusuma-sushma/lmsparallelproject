				...
				do {
					...
					case 1:
						try {
							...
							if (adminLogin != null && (adminLogin.getRole().equalsIgnoreCase("admin"))) {
								...
								do {
									...
									case 1:
										...
										try {
											...
											if (add) {
												...
												System.err.println("User's email or id already exists");
											}
										} catch (Exception e) {
											e.printStackTrace();
									...
									case 3:
										try {
											...
											System.out.println(
													String.format("%-10s %-15s %-25s %-15s %-25s %-15s %-10s -%s",
															"USERID", "USERNAME", "EMAIL", "PASSWORD", "DEPARTMENT",
															"ROLE", "FINE", "BOOKSBORROWED"));

											for (UserInformation user : listUsers) {
												System.out.println(String.format(
														"%-10s %-15s %-25s %-15s %-25s %-15s %-10s -%s",
														user.getUserId(), user.getUsername(), user.getEmail(),
														user.getPassword(), user.getDepartment(), user.getRole(),
														user.getFine(), user.getNoOfBooksBorrowed()));
											}
											System.out.println("*********************");
										} catch (Exception e) {
									...
									case 4:
										try {
											...
											System.out.println(String.format("%-15s %-30s %-20s %-20s %-20s %s",
									...
									case 6:
										try {
											...
											boolean isBookIssued = adminService
													.issueBook(Integer.parseInt(issueBookId));
											if (isBookIssued) {
												...
												System.err.println("Invalid Request Id, Book cannot be issued to user");
											}
					...
					case 2:
						try {
							...
							if (userLogin != null && (userLogin.getRole().equalsIgnoreCase("student"))) {
								...
								do {
									...
									case 2:
										...
										System.out.println(String.format("%-5s %-30s %-20s %-20s %-20s %s", "BOOKID",
												"BOOKNAME", "BOOKCATEGORY", "BOOKAUTHOR", "BOOKPUBLISHER",
												"BOOKAVALIABLITY"));

										for (BooksInformation book1 : listBook) {
											System.out.println(String.format("%-5s %-30s %-20s %-20s %-20s %s",
													book1.getBookId(), book1.getBookName(), book1.getBookCategory(),
													book1.getBookAuthor(), book1.getBookPublisher(),
													book1.isBookAvailable()));
										}
									...
									case 4:
										...
										try {
											userService.returnBook(Integer.parseInt(returnUser),
